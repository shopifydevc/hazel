...
export const SidebarNavigationSlim = ({
	...
	const mainSidebar = (
		...
		>
			<div
				className={cx(
					"flex w-auto flex-col justify-between rounded-xl pt-5 ring-1 ring-secondary ring-inset transition duration-300",
					...
					{items.map((item) => (
						<li key={item.label}>
							<NavItemButton
								...
								label={item.label || ""}
								icon={item.icon}
								onClick={(_e) => setCurrentItem(item)}
							/>
						</li>
					))}
				</ul>
		...
		<AnimatePresence initial={false}>
			{isSecondarySidebarVisible && (
				<motion.div
					...
					className={cx(
						"relative h-full overflow-y-auto bg-primary",
						!(hideBorder || hideRightBorder) && "border-r-[1.5px]",
					)}
				>
					<div
						style={{ width: SECONDARY_SIDEBAR_WIDTH }}
						className="flex h-full flex-col px-4 pt-6 pb-5"
					>
						<h3 className="font-semibold text-brand-secondary text-sm">{currentItem.label}</h3>
						<ul className="mt-2">
							{currentItem.items?.map((item, _index) => (
