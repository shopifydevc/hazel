	...
	return (
		<li
			...
			className={cx(
				...
				msg.user?.me ? "self-end pl-10" : "pr-8 lg:pr-10",
				props.className,
			)}
		>
			{msg.user && !msg.user.me && (
				<Avatar src={msg.user.avatarUrl} alt={msg.user.name!} size="md" status={msg.user.status} />
			)}
			...
			<div className="flex min-w-0 flex-1 flex-col gap-1.5">
					...
					<div className="flex items-center gap-2">
						...
						{(msg.sentAt || msg.status) && (
							<div className="flex items-center gap-0.5">
								{msg.sentAt && <span className="text-xs text-tertiary">{msg.sentAt}</span>}
				...
				{msg.text ? (
					...
					>
						{msg.reply && (
							<div className="relative mb-1.5 rounded-lg bg-primary px-3 py-2 text-sm text-tertiary ring-1 ring-secondary ring-inset before:pointer-events-none before:absolute before:inset-0 before:rounded-[inherit] before:border-l-[3px] before:border-brand">
								{msg.reply.text}
							</div>
						)}
						...
						{msg.image && (
							<img
								src={msg.image.src}
								alt={msg.image.alt}
								className="mt-1 mb-1.5 w-full rounded-md object-cover outline-1 -outline-offset-[0.5px] outline-black/10"
							/>
						)}

						{msg.urlPreview && (
							<div className="mt-1 mb-1.5 flex items-start gap-1.5 rounded-lg bg-primary p-2 pr-3 ring-1 ring-secondary ring-inset">
								<Link03 className="mt-0.5 size-4 shrink-0 text-fg-quaternary" />
								<div className="min-w-0 flex-1">
									<p className="w-full truncate text-sm font-medium text-secondary">
										{msg.urlPreview.title}
									</p>
									<p className="w-full truncate text-sm text-tertiary">
										{msg.urlPreview.description}
									</p>
								</div>
							</div>
						)}
					...
					<div className="flex flex-col gap-1.5">
						...
						<div className="flex items-center gap-1">
							...
							<p className="text-sm text-tertiary">{msg.image.size}</p>
						</div>
					</div>
				) : msg.audio ? (
					<div className="group/msg relative flex items-center gap-2 rounded-lg rounded-tl-none bg-primary p-3 ring-1 ring-secondary">
						...
						>
							<svg
								...
								viewBox="0 0 16 16"
								fill="none"
								className="translate-x-[1px] text-fg-white"
							>
								<path
						...
						<svg
							...
							viewBox="0 0 206 34"
							fill="none"
							className="flex-1 text-fg-brand-primary_alt"
						>
							<path
