...
export const MessageItem = ({ msg, showUserLabel = true, ...props }: MessageItemProps) => {
    const renderActions = () => (
        <div className="dark-mode -bottom-5 absolute right-2 z-1 flex gap-1.5 rounded-lg bg-primary_alt px-2 py-1.5 opacity-0 shadow-xl transition duration-100 ease-linear group-hover/msg:opacity-100">
            <button
            ...
            <article className="flex min-w-0 flex-1 flex-col gap-1.5">
                {((msg.user && showUserLabel) || msg.sentAt || msg.status) && (
                    <header className="flex items-center gap-2">
                        ...
                        {msg.user && showUserLabel && (
                            <cite className="flex-1 truncate font-medium text-secondary text-sm not-italic">{msg.user.me ? "You" : msg.user.name}</cite>
                        )}
                        ...
                        {(msg.sentAt || msg.status) && (
                            <div className="flex items-center gap-0.5">
                                {msg.sentAt && (
                                    <time className="text-tertiary text-xs" dateTime={msg.sentAt}>
                                        {msg.sentAt}
                                    </time>
                                )}
                ...
                {msg.text ? (
                        ...
                        {msg.image && (
                            <figure className="mt-1 mb-1.5">
                                <img
                                    src={msg.image.src}
                                    alt={msg.image.alt}
                                    className="-outline-offset-[0.5px] w-full rounded-md object-cover outline-1 outline-black/10"
                                />
                            </figure>
                        )}
                        ...
                        {msg.urlPreview && (
                            <aside className="mt-1 mb-1.5 flex items-start gap-1.5 rounded-lg bg-primary p-2 pr-3 ring-1 ring-secondary ring-inset">
                                ...
                                <div className="min-w-0 flex-1">
                                    <p className="w-full truncate font-medium text-secondary text-sm">{msg.urlPreview.title}</p>
                    ...
                    <figure className="flex flex-col gap-1.5">
                        ...
                        <figcaption className="flex items-center gap-1">
                            <span className="min-w-0 flex-1 truncate font-medium text-secondary text-sm">{msg.image.name}</span>
                    ...
                    <div className="group/msg relative flex items-center gap-2 rounded-lg rounded-tl-none bg-primary p-3 ring-1 ring-secondary">
                        ...
                        <p className="text-tertiary text-xs">{msg.audio.duration}</p>
                ...
                {msg.attachment && (
                    <div className="group/msg relative flex gap-3 rounded-lg rounded-tl-none bg-primary px-3.5 py-2.5 ring-1 ring-secondary">
                        ...
                        <FileIcon type={msg.attachment.type} theme="dark" className="not-dark:hidden size-10" />

                        <div className="min-w-0 flex-1">
                            <p className="truncate font-medium text-secondary text-sm">{msg.attachment.name}</p>
                    ...
                    <ul className="flex justify-end gap-1">
                        {msg.reactions?.map((reaction) => (
                            ...
                            >
                                ...
                                {reaction.count > 1 && <span className="font-medium text-secondary text-sm">{reaction.count}</span>}
                            </li>
                        ))}
                    </ul>
                )}
