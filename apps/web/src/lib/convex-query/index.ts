...
function isConvexQuery(
	queryKey: readonly any[],
): queryKey is ["convexQuery", FunctionReference<"query">, Record<string, any>, {}] {
	return queryKey.length >= 2 && queryKey[0] === "convexQuery"
}

function isConvexAction(
	queryKey: readonly any[],
): queryKey is ["convexAction", FunctionReference<"action">, Record<string, any>, {}] {
	return queryKey.length >= 2 && queryKey[0] === "convexAction"
}

function hash(queryKey: ["convexQuery", FunctionReference<"query">, Record<string, any>, {}]): string {
	return `convexQuery|${getFunctionName(queryKey[1])}|${JSON.stringify(convexToJson(queryKey[2]))}`
}

export interface ConvexQueryClientOptions extends ConvexSolidClientOptions {
...
export class ConvexQueryClient {
	...
	constructor(
		...
		if (typeof client === "string") {
			this.convexClient = new ConvexSolidClient(client, options)
		} else {
	...
	subscribeInner(queryCache: QueryCache): () => void {
		...
		return queryCache.subscribe((event) => {
			...
			switch (event.type) {
				...
				// Runs when a createQuery mounts
				case "observerAdded": {
					break
				}
				// Runs when a createQuery unmounts
				case "observerRemoved": {
					if (event.query.getObserversCount() === 0) {
						// The last createQuery subscribed to this query has unmounted.
				...
				// Fires once per createQuery hook
	 ...
	 * Convex queries.
	 */
	queryFn(otherFetch: QueryFunction<unknown, QueryKey> = throwBecauseNotConvexQuery) {
		return async <ConvexQueryReference extends FunctionReference<"query", "public">>(
	 ...
	 *
	 * ```
	 * createQuery(() => client.queryOptions(api.foo.bar, args))
	 * ```
	 *
 ...
 *
 * ```
 * createQuery(() => convexQuery(api.foo.bar, args))
 * ```
 *
 * If you need to specify other options spread it:
 * ```
 ...
 * Note that Convex actions are not live updating: they follow the normal solid-query
 * semantics of refreshing on window focus, etc.
 *
 * ```
 * createQuery(() => convexAction(api.weather.now, { location: "SF" }))
 * ```
 *
 * If you need to specify other options spread it:
 * ```
...
function throwBecauseNotConvexQuery(context: QueryFunctionContext<ReadonlyArray<unknown>>) {
