/**
 * Sampled OTLP Tracer - Based on @effect/opentelemetry OtlpTracer with head-based sampling
 *
 * Key differences from the original:
 * - Adds `sampleRate` option (0.0 to 1.0)
 * - Root spans are sampled probabilistically based on sampleRate
 * - Child spans inherit sampling decision from parent
 * - Non-sampled spans are not exported
 */
...
/**
 * Create a sampled OTLP tracer
 */
export const make: (options: {
	readonly url: string
	readonly sampleRate: number // 0.0 to 1.0 - probability of sampling root spans
	readonly resource?: {
...
}) => Effect.Effect<Tracer.Tracer, never, HttpClient.HttpClient | Scope.Scope> = Effect.fnUntraced(function* (options) {
	...
	const sampleRate = Math.max(0, Math.min(1, options.sampleRate))

	return Tracer.make({
		span(name, parent, context, links, startTime, kind) {
			// Determine sampling decision:
			// - If parent exists, inherit parent's sampling decision
			// - If no parent (root span), use probabilistic sampling
			const sampled = Option.isSome(parent) ? parent.value.sampled : Math.random() < sampleRate
...
/**
 * Create a layer that provides a sampled OTLP tracer
 */
