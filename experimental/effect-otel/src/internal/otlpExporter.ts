/**
 * OTLP Exporter - Copied from @effect/opentelemetry with minor modifications
 * @internal
 */
...
export const make: (options: {
	...
	Effect.fnUntraced(function* (options) {
		...
		const runExport = Effect.suspend(() => {
			...
			return client.execute(HttpClientRequest.bodyUnsafeJson(request, options.body(items))).pipe(
				Effect.asVoid,
